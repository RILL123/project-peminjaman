...
<body class="bg-perpusku4">
    ...
    <div id="mainContent" class="md:ml-64 transition-all duration-300 min-h-screen">
        <div class="max-w-6xl mx-auto px-4 py-8">
                                                ...
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                        ...
                                                        echo "<td class='py-2 px-3 flex gap-2'>
                                                            <button 
                                                                ...
                                                                class='bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded shadow text-xs font-semibold transition mr-2'
                                                                data-judul='{$row['judul']}'
                                                                data-kategori='{$row['kategori']}'
                                                                data-penulis='{$row['penulis']}'
                                                                data-cover='{$row['cover']}'
                                                            >Detail</button>
                                                            <button onclick=\"setReturnData('{$row['id_peminjaman']}', '{$row['nama_user']}', '{$row['id_user']}', '{$row['id_buku']}', '{$row['judul']}', '{$row['kategori']}', '{$row['penulis']}', '{$row['cover']}', '{$row['tanggal_pinjam']}', '{$row['tanggal_kembali']}')\" class='bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded shadow text-xs font-semibold transition'>Kembalikan Buku</button>
                                                        </td>";
                                                        echo "</tr>";
                                                        $no++;
                                                }
            ...
            <div id="strukSection" class="hidden bg-white rounded-2xl shadow-xl p-10 max-w-lg mx-auto mt-12 border border-green-200 animate-fadeIn">
                ...
                <form id="returnForm" action="../../controller/aksi_peminjaman.php" method="POST">
                    <input type="hidden" name="id_peminjaman" id="return_id">
                    <input type="hidden" name="aksi" value="kembalikan">
                    <input type="hidden" name="id_user" id="return_id_user">
                    <input type="hidden" name="id_buku" id="return_id_buku">
                    <input type="hidden" name="judul" id="return_judul">
    ...
    <script>
        ...
        function setReturnData(id, nama_user, id_user, id_buku, judul, kategori, penulis, cover, tglPinjam, tglKembali) {
            ...
            document.getElementById('return_id_user').value = id_user;
            document.getElementById('return_id_buku').value = id_buku;
            document.getElementById('return_judul').value = judul;
